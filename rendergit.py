...
def collect_files(
    repo_root: Path, max_bytes: int, use_gitignore: bool = False
) -> list[FileInfo]:
    ...
    for p in sorted(repo_root.rglob("*"), key=cmp_to_key(custom_sort_key)):
        if p.is_symlink():
            continue
        if p.is_file():
...
def generate_tree_fallback(root: Path) -> str:
...
def try_tree_command(root: Path) -> str:
    ...
    try:
        ...
        cmd2 = subprocess.check_output(
            ("tree", "--dirsfirst", "--fromfile"), stdin=cmd1.stdout, text=True
        )
        cmd1.wait()
        return cmd2
    except Exception:
...
def render_notebook_html(nb_path: Path) -> str:
...
def generate_cxml_text(infos: list[FileInfo], repo_dir: Path) -> str:
...
def generate_advanced_stats(infos: list[FileInfo]) -> str:
    ...
    for file_info in rendered:
        ext = Path(file_info.rel).suffix.lower() or "no-extension"
        ext_stats[ext] += 1
    ...
    <div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem; margin: 1.5rem 0;">
      <div class="stat-card" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; padding: 1.5rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-md);">
        ...
        <div>Largest File: <strong>{
        bytes_human(max((f.size for f in rendered), default=0))
    }</strong></div>
      </div>
      ...
      <div class="stat-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 1.5rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-md);">
        ...
        <div>Nested Files: <strong>{
        sum(count for depth, count in depth_stats.items() if depth > 0)
    }</strong></div>
      </div>
    </div>
        ...
        "".join(
          ...
          '''
            for lang, stats in sorted(
                lang_stats.items(), key=lambda x: x[1]["count"], reverse=True
            )[:8]
        )
    }
        </div>
...
def custom_sort_key(filename1: str | Path | FileInfo, filename2: str | Path | FileInfo) -> int:
    if isinstance(filename1, Path):
        filename1 = str(filename1)
    elif isinstance(filename1, FileInfo):
        filename1 = filename1.rel

    if isinstance(filename2, Path):
        filename2 = str(filename2)
    elif isinstance(filename2, FileInfo):
        filename2 = filename2.rel

    parts1 = filename1.lower().split('/')
    parts2 = filename2.lower().split('/')

    for i, (p1, p2) in enumerate(zip(parts1, parts2)):
        if p1 != p2:
            is_last_p1 = (i == len(parts1) - 1)
            is_last_p2 = (i == len(parts2) - 1)
            if is_last_p1 and is_last_p2:
                if p1.startswith('.'):
                    p1 = p1[1:]
                if p2.startswith('.'):
                    p2 = p2[1:]

                # Split the part into parts based on numbers and non-numbers
                # so 1_a.txt is lower than 10_b.txt
                # return [int(s) if s.isdigit() else s for s in re.split(r'(\d+)', p1)] < [int(s) if s.isdigit() else s for s in re.split(r'(\d+)', p2)]
                return (p1 > p2) - (p1 < p2)
...
#     if not filename.rel.split("/")[-1].startswith('.'):
#         filename = filename.rel.split("/")[-1].lower()
#     else:
#         filename = filename.rel.split("/")[-1][1:].lower()

#     # Split the filename into parts based on numbers and non-numbers
#     parts = [int(s) if s.isdigit() else s for s in re.split(r'(\d+)', filename)]
#     return parts
...
def generate_tree_items(tree, path_prefix="", depth=0):
    ...
    for dir_name in sorted(key for key in tree.keys() if key != "_files"):
        ...
        items.append(
            f'<li class="toc-directory" data-depth="{depth}"><span class="directory-name">{indent}{folder_icon} {html.escape(dir_name)}/</span></li>'
        )
        items.extend(generate_tree_items(tree[dir_name], dir_path, depth + 1))

    # Then add files in current directory
    if "_files" in tree:
        for file_info in sorted(tree["_files"], key=cmp_to_key(custom_sort_key)):
            ...
            items.append(
                f'<li class="toc-file" data-depth="{depth + 1}"><a href="#file-{anchor}">{indent}{file_icon} {html.escape(filename)} <span class="muted">({bytes_human(file_info.size)})</span></a></li>'
            )
...
def build_html(
    repo_url: str, repo_dir: Path, head_commit: str, infos: list[FileInfo]
) -> str:
    ...
    # Sort files to be rendered
    rendered = sorted(rendered, key=cmp_to_key(custom_sort_key))
    ...
    # for dir_files in file_tree.values():
    #     if "_files" in dir_files:
    #         dir_files["_files"] = sorted(dir_files["_files"], key=custom_sort_key)
    ...
    for i in rendered:
        ...
        if ext in MARKDOWN_EXTENSIONS:
            ...
            file_icon = "📱"
        elif ext in {".dockerfile", ".dockerignore"} or p.name.lower() in {
            "dockerfile",
            "docker-compose.yml",
            "docker-compose.yaml",
        }:
        ...
        try:
            if ext in NOTEBOOK_EXTENSIONS:
                ...
                if ext in MARKDOWN_EXTENSIONS:
                    body_html = (
                        f'<div class="markdown-content">{render_markdown_text(text)}</div>'
                    )
                else:
...
def derive_temp_output_path(repo_url: str) -> Path:
...
def main() -> int:
    ...
    input_path = args.repo_path_or_url
    is_local = os.path.exists(input_path)

    if args.out is None:
        args.out = str(derive_temp_output_path(input_path))

    tmpdir = None
    if is_local:
        repo_dir = Path(input_path).resolve()
        repo_url = str(repo_dir)
        head = git_head_commit(str(repo_dir))
        print(f"📁 Using local repo: {repo_dir}", file=sys.stderr)
    else:
